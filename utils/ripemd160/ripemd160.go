...
func Ripemd160(hash []byte) []byte {
	hash160Writer := crypto.RIPEMD160.New()
	_, _ = hash160Writer.Write(hash[:]) // return len(), error: the error always nil

	return hash160Writer.Sum(nil) // the param will append to the result
}
